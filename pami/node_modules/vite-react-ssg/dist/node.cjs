...
const { version } = JSON.parse(
  node_fs.readFileSync(new URL("../package.json", (typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (document.currentScript && document.currentScript.src || new URL('node.cjs', document.baseURI).href)))).toString()
);
...
async function render(routesOrApp, request, styleCollector, basename) {
  ...
  if (Array.isArray(routesOrApp)) {
    ...
    inner = /* @__PURE__ */ React__default.createElement(StaticRouterProvider, { router, context });
  } else {
    inner = routesOrApp;
  }
  let app = /* @__PURE__ */ React__default.createElement(reactHelmetAsync.HelmetProvider, { context: helmetContext }, inner);
  if (styleCollector)
