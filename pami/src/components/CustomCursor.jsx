...
import { useIsTouch, usePrefersReducedMotion } from '../hooks/useMediaQuery.js';
...
export default function CustomCursor() {
  ...
  useEffect(() => {
    ...
    const onMove = (e) => {
      ...
      // The hover target test: interactive elements opt in via [data-cursor]
      // or are native links/buttons.
      const el = e.target;
  ...
  return (
    <div aria-hidden="true" className="pointer-events-none fixed inset-0 z-[100]" style={{ opacity: visible ? 1 : 0 }}>
      <div
        ref={ringRef}
        className="fixed left-0 top-0 -ml-4 -mt-4 h-8 w-8 rounded-full border border-ink transition-[width,height,background-color,opacity] duration-300 ease-editorial"
        style={{
          width: hovering ? 56 : 32,
          height: hovering ? 56 : 32,
          marginLeft: hovering ? -28 : -16,
          marginTop: hovering ? -28 : -16,
          backgroundColor: hovering ? 'rgba(22,20,15,0.06)' : 'transparent',
        }}
      />
      <div
        ref={dotRef}
